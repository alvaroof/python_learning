...
from classes import BankAccount, Flavor, IceCream, MinHeap, Month, MonthDelta, Row, Size, SuperMap
...
class SuperMapTests(unittest.TestCase):
    ...
    def test_where_method(self):
        ...
        self.assertEqual(set(matches), {item for item in few_items if item.color == "pink"})
    ...
    def test_time_efficient_lookups(self):
        many_items = [
            Item(i, "".join(next(names)), next(colors), random.randint(0, 5)) for i in range(2_000)
        ]
        mapping = SuperMap(many_items, indexes=("id", "color"))
        with Timer() as manual_lookup:
            items1 = {item for item in many_items if item.color == "pink"}
...
class FlavorTests(unittest.TestCase):
    ...
    def test_specifying_ingredients(self):
        ...
        flavor = Flavor("chocolate", ingredients=["milk", "sugar", "vanilla", "chocolate"])
        self.assertEqual(
            flavor.ingredients,
